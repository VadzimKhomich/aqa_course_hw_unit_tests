...
async function getUsersInfo() {
  try {
    const [users, albums, photos] = await Promise.all([
      ...
      fetch('https://jsonplaceholder.typicode.com/albums').then((data) => data.json()),
      fetch('https://jsonplaceholder.typicode.com/photos').then((data) => data.json()),
    ]);
    users.forEach((user, index) => {
      ...
      const userAlbumsInfo = usersAlbums.map((album) => {
        const countPhoto = photos.filter((photo) => photo.albumId === album.id).length;
        return `${album.title} (${countPhoto} photos)`;
      });

      userAlbumsInfo.forEach((albumInfo) => (resultString += `\t  ${albumInfo}\n`));
